...
public:
	...
	matrix<num_t> sign() const;
	// 返回矩阵各元素的符号

	matrix<num_t> number_power_as_exponent(num_t) const;
	matrix<num_t> number_exp() const;
	// 以数为底、以矩阵各元素为指数的乘方
	matrix<num_t> number_power_as_base(num_t) const;
	// 以矩阵各元素为底、以数为指数的乘方
	matrix<num_t> number_multiply(const matrix<num_t>&) const;
	matrix<num_t> number_reciprocal() const;
	// 对矩阵各个元素求倒数
	matrix<num_t> number_sin() const;
	matrix<num_t> number_cos() const;
	matrix<num_t> number_function(std::function<num_t(const num_t)>) const;
	// 对矩阵的各个元素使用自定义函数计算
...
template<class num_t, int n_row, int n_column> class register_matrix
		// 尽可能将矩阵数据的存储放到寄存器上
{
public:
	num_t val[n_row * n_column];

	register_matrix();
	register_matrix(num_t*);
	register_matrix(num_t);
	// 传入行数、列数，读入数据
	register_matrix(const matrix<num_t>&);
	register_matrix(const register_matrix<num_t, n_row, n_column>&);
	~register_matrix();
	...
	register_matrix<num_t, n_row, n_column>& operator=(const register_matrix<num_t, n_row, n_column>&);
	register_matrix<num_t, n_row, n_column>& operator=(num_t);
	bool operator==(const register_matrix<num_t, n_row, n_column>&) const;

	register_matrix<num_t, n_row, n_column>& operator+=(const register_matrix<num_t, n_row, n_column>&);
	register_matrix<num_t, n_row, n_column>& operator+=(num_t);
	register_matrix<num_t, n_row, n_column> operator+(const register_matrix<num_t, n_row, n_column>&) const;
	register_matrix<num_t, n_row, n_column> operator+(num_t) const;

	register_matrix<num_t, n_row, n_column>& operator*=(num_t);
	template <int n_column2> register_matrix<num_t, n_row, n_column> operator*(register_matrix<num_t, n_column, n_column2>) const;
	register_matrix<num_t, n_row, n_column> operator*(num_t) const;

	register_matrix<num_t, n_row, n_column> operator-() const;
	register_matrix<num_t, n_row, n_column> operator-(const register_matrix<num_t, n_row, n_column>&) const;
	register_matrix<num_t, n_row, n_column>& operator-=(const register_matrix<num_t, n_row, n_column>&);
	register_matrix<num_t, n_row, n_column> operator-(const num_t);

	register_matrix<num_t, n_row, n_column>& operator/=(num_t);
	register_matrix<num_t, n_row, n_column> operator/(num_t) const;

	register_matrix<num_t, n_column, n_row> transposition() const;
	// 返回矩阵的转置
	num_t sum() const;
	// 返回矩阵各元素的和
	register_matrix<num_t, n_row, n_column> sign() const;
	// 返回矩阵各元素的符号

	register_matrix<num_t, n_row, n_column> number_power_as_exponent(num_t) const;
	register_matrix<num_t, n_row, n_column> number_exp() const;
	// 以数为底、以矩阵各元素为指数的乘方
	register_matrix<num_t, n_row, n_column> number_power_as_base(num_t) const;
	// 以矩阵各元素为底、以数为指数的乘方
	register_matrix<num_t, n_row, n_column> number_multiply(const register_matrix<num_t, n_row, n_column>&) const;
	register_matrix<num_t, n_row, n_column> number_reciprocal() const;
	// 对矩阵各个元素求倒数
	register_matrix<num_t, n_row, n_column> number_sin() const;
	register_matrix<num_t, n_row, n_column> number_cos() const;
	register_matrix<num_t, n_row, n_column> number_function(std::function<num_t(const num_t)>) const;
	// 对矩阵的各个元素使用自定义函数计算

	static register_matrix<num_t, n_row, n_column> linspace(num_t, num_t, int);
	// 传入上下限、个数，生成等差的列向量
	static register_matrix<num_t, n_row, n_column> random(int, int);
	// 生成指定大小的随机数矩阵
	static register_matrix<num_t, n_row, n_column> vander(register_matrix<num_t, n_row, 1>, int);
...
template<class num_t> matrix<num_t> matrix<num_t>::col(int i) const
{
	if (i < 0 || i >= size[1])
...
template<class num_t> int matrix<num_t>::n_col() const
{
	return size[1];
}
...
{
    matrix<num_t> rtn(this->n_row(), this->n_col());
...
template<class num_t> matrix<num_t> matrix<num_t>::number_power_as_exponent(num_t a) const
{
...
template<class num_t> matrix<num_t> matrix<num_t>::number_exp() const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
		ans.val[i] = exp(val[i]);
...
template<class num_t> matrix<num_t> matrix<num_t>::number_power_as_base(num_t b) const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
...
template<class num_t> matrix<num_t> matrix<num_t>::number_multiply(const matrix<num_t>& b) const
{
	if (memcmp(size, b.size, sizeof(int) * 2))
		throw("matrix size not match");
	matrix<num_t> ans(size);
...
template<class num_t> matrix<num_t> matrix<num_t>::number_reciprocal() const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
		ans.val[i] = 1 / val[i];
...
template<class num_t> matrix<num_t> matrix<num_t>::number_sin() const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
		ans.val[i] = sin(val[i]);
...
template<class num_t> matrix<num_t> matrix<num_t>::number_cos() const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
		ans.val[i] = cos(val[i]);
...
template<class num_t> matrix<num_t> matrix<num_t>::number_function(std::function<num_t(num_t)> f) const
{
	matrix<num_t> ans(size);
	for (int i = 0; i < length(); i++)
		ans.val[i] = f(val[i]);
...
template<class num_t, int n_row, int n_column> register_matrix<num_t, n_row, n_column>::register_matrix(){}
template<class num_t, int n_row, int n_column> register_matrix<num_t, n_row, n_column>::register_matrix(num_t *value)
{
#ifdef REGISTER_MATRIX_DEBUG
